...
#define ACTIVE_REGION LORAMAC_REGION_AS923 // ROY
 ...
 */
static void PrepareTxFrame( uint8_t port, char * dataBuf )
{
    // AppDataSize = 4;
    // AppData[0] = 0x00;
    // AppData[1] = 0x01;
    ...
    // ROY
    strncpy((char*) AppData, dataBuf, sizeof(AppData) - 1);
    AppData[sizeof(AppData) - 1] = '\0';
}
        ...
        switch( DeviceState )
        {
            ...
            case DEVICE_STATE_SEND:
            {
                if( NextTx == true )
                {
                    ...
                    snprintf(dataBuf, sizeof(dataBuf), "counted: %d", counter++);
                    
                    PrepareTxFrame( AppPort, dataBuf );
